...
class SceneParser {
 public:
  SceneParser(IndexerType idxType);
 ...
 private:
  ...
  std::unique_ptr<SceneNode> ParseSceneNode(std::vector<HittableBase*>& objects);
  void ParseTransform(Transform& transform);
  void ParseComponent(const std::string& type, SceneNode& node, std::vector<HittableBase*>& objects);
  void ParseCamera();
  void ParseLightComponent(SceneNode& node);
  void ParseMaterialComponent(SceneNode& node);
  void ParseTracingComponent(SceneNode& node, std::vector<HittableBase*>& objects);
