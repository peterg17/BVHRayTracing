...
namespace GLOO {
SceneParser::SceneParser(IndexerType idxType) {
  idx_type_ = idxType;
}
...
std::unique_ptr<Scene> SceneParser::ParseScene(const std::string& filename) {
  ...
  std::vector<HittableBase*> objects_vec = std::vector<HittableBase*>();
  std::vector<HittableBase*>& objects = objects_vec;
  ...
  while (fs_ >> token) {
    ...
    if (token == "Background") {
      ...
      scene = make_unique<Scene>(ParseSceneNode(objects));
    } else if (token != "}") {
  ...
  // std::cout << "Gets to the end ParseScene, about to build BVH!" << std::endl;
  // scene->GetRootNode().BuildBVH(objects);
...
std::unique_ptr<SceneNode> SceneParser::ParseSceneNode( std::vector<HittableBase*>& objects) {
  ...
  while (token != "}") {
    fs_ >> token;
    if (token == "Node") {
      node->AddChild(ParseSceneNode(objects));
      ...
      ParseComponent(type, *node, objects);
    } else if (token != "}") {
...
void SceneParser::ParseComponent(const std::string& type, SceneNode& node, std::vector<HittableBase*>& objects) {
  if (type == "Material") {
    ParseMaterialComponent(node);
  } else if (type == "Light") {
    ParseLightComponent(node);
  } else if (type == "Object") {
    ParseTracingComponent(node, objects);
  } else {
...
void SceneParser::ParseTracingComponent(SceneNode& node, std::vector<HittableBase*>& objects) {
  std::string token;
  fs_ >> token;
  Assert(token, "{");
  ...
  // std::cout << "Adding object to objects vector!" << std::endl;
  // HittableBase *obj_ptr = object.get();
